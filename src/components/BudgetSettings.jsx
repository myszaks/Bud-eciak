...
export default function BudgetSettings({
  ...
  useEffect(() => {
    ...
    console.log(
      "[BudgetSettings] Wykryto zmianę budżetu z",
      budgetId,
      "na",
      selectedBudget.id
    );
    ...
    if (selectedBudget.is_owner) {
      ...
      console.log(
        "[BudgetSettings] Nowy budżet nie jest Twój - powrót do Dashboard"
      );
      navigate("/", { replace: true });
    }
  }, [selectedBudget?.id, selectedBudget?.is_owner, budgetId, navigate]);

  // Fetch shares (returns data) — keeps email lookup inside, so callers can run it in parallel
  ...
  // Wrapper that sets state (reuse in multiple places)
  async function fetchShares() {
    const result = await fetchSharesData();
    setShares(result);
  }
  ...
  const fetchBudgetData = useCallback(async () => {
    ...
    try {
      ...
      const [{ data: budgetData, error: budgetError }, sharesData] =
        await Promise.all([budgetPromise, sharesPromise]);
      ...
      if (budgetData.owner_id !== session.user.id) {
        console.log(
          "[BudgetSettings] Nie jesteś właścicielem budżetu:",
          budgetId
        );
      ...
      if (!selectedBudget || selectedBudget.id !== budgetData.id) {
        console.log(
          "[BudgetSettings] Aktualizuję selectedBudget do:",
          budgetData.id
        );
        ...
        console.log(
          "[BudgetSettings] Pomijam aktualizację selectedBudget - ten sam budżet"
        );
      }
  ...
  function requestDeleteBudget(budget) {
    openModal("confirmDeleteBudget", {
      budget,
  ...
  async function shareBudget(e) {
    ...
    try {
      ...
      await fetchShares();
      toast.success(
        `Budżet został udostępniony użytkownikowi ${trimmedEmail}!`
      );
    } catch (error) {
  ...
  function requestRemoveShare(shareToRemove, bugdet) {
    openModal("confirmDelete", {
      title: "Cofnij dostęp użytkownikowi",
      description: (
        <>
          Czy na pewno chcesz cofnąć dostęp użytkownikowi{" "}
          <strong className="text-text">{shareToRemove.user_email}</strong>?
        </>
      ),
      confirmText: "Usuń",
      confirmVariant: "danger",
      onConfirm: async () => {
        await deleteShare(shareToRemove.id, bugdet.id);
      },
    });
  }

  async function deleteShare(shareId, budgetId) {
    if (!shareId) return;
  ...
  function requestPermissionChange(share) {
    const newLevel = share.access_level === "view" ? "edit" : "view";

    openModal("changePermission", {
      currentLevel: share.access_level,
      newLevel,
      userEmail: share.user_email,
      confirmText: "Zmień",
      onConfirm: async () => {
        await permissionChange(share.id, newLevel);
      },
    });
  }

  async function permissionChange(shareId, newLevel) {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-8 text-center">
          ...
          <p className="text-gray-400 mb-6">
            {error ||
              "Budżet nie został znaleziony lub nie masz do niego dostępu."}
          </p>
          <button
        ...
        {editMode ? (
          <form onSubmit={updateBudget} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 bg-dark-card border border-dark-border rounded-lg text-text focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all"
                required
              />
            </div>
            <div>
              ...
              <textarea
                ...
                rows="3"
                className="w-full px-4 py-2.5 bg-dark-card border border-dark-border rounded-lg text-text focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 resize-none transition-all"
              />
            </div>
            <div className="flex gap-3">
              ...
              <button
                ...
                className="bg-dark-border text-text py-2.5 px-6 rounded-lg hover:bg-dark-hover transition-colors font-medium"
              >
                Anuluj
              </button>
            </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="text-text font-medium">{budget.name}</p>
            </div>
            {budget.description && (
              <div>
                <p className="text-sm text-gray-400 mb-1">Opis</p>
                <p className="text-text">{budget.description}</p>
              </div>
            )}
            <div>
      ...
      {isOwner && (
        <div className="bg-dark-surface border border-dark-border rounded-xl p-6 shadow-lg">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-xl font-bold text-text">Udostępnianie</h2>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-dark-card border border-dark-border rounded-lg text-text placeholder-gray-500 focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all"
                  required
                />
              </div>
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full px-4 py-2.5 bg-dark-card border border-dark-border rounded-lg text-text focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all appearance-none cursor-pointer"
                  >
                ...
                {shares.map((share) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => requestPermissionChange(share)}
                        className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                          share.access_level === "edit"
                            ? "bg-blue-500/20 text-blue hover:bg-blue-500/30"
                            : "bg-gray-500/20 text-gray hover:bg-gray-500/30"
                        }`}
                      >
                        {share.access_level === "edit" ? "Edycja" : "Podgląd"}
                      </button>
                      <button
                        onClick={() => requestRemoveShare(share, budget)}
        ...
        <div className="bg-dark-surface border border-red-500/30 rounded-xl p-6 shadow-lg">
          ...
          <div className="space-y-4">
            <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
              <h3 className="font-medium text-red-500 mb-2">Usuń budżet</h3>
              <p className="text-sm text-gray-400 mb-4">
                Usunięcie budżetu jest nieodwracalne. Wszystkie wydatki, wpływy
                i udostępnienia zostaną trwale usunięte.
              </p>
              <button
                onClick={() => requestDeleteBudget(budget)}
